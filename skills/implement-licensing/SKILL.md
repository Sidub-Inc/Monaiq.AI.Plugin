...
|--------------|---------|----------------|
| Package install fails | NuGet/npm error during `dotnet add package` or `npm install` | Check network connectivity and package source configuration. Retry with `--source nuget.org` for .NET or clear npm cache for React. |
| Credential retrieval fails | `profile` tool returns error on step 2 | Verify session is active via `register_or_login`. Re-authenticate if session expired. |
| Config binding fails | `LicensingConfigurationException` at runtime | Verify `appsettings.json` section name matches `"LicensingServiceOptions"` exactly. Check that `EncodedCredential` is not empty or malformed. |
| DI registration fails | Build error on `AddSidubLicensing()` | Verify package is installed. Check that `using Sidub.Licensing.Client` and `using Sidub.Licensing.Client.Extensions` are present. |
| GetAuthorization returns null | License validation fails at runtime | Check credential validity — retrieve fresh `EncodedCredential` from `profile` tool. Verify `LicenseServiceUri` is correct. |
